...
// node_modules/.pnpm/isomorphic-ws@5.0.0_ws@8.16.0/node_modules/isomorphic-ws/browser.js
var ws = null;
...
// src/types.ts
var tunnelHeaderSchema = z.union([z.undefined(), z.string(), z.string().array()]);
var tunnelRequestSchema = z.object({
...
var tunnelResponseSchema = z.object({
...
var helloSchema = z.object({ type: z.literal("hello") });
var tailSchema = z.union([tunnelRequestSchema, helloSchema]);
var headSchema = z.union([tunnelResponseSchema, helloSchema]);
...
var TunnelClient = class {
  constructor(_ws) {
    ...
    __publicField(this, "_ev", new EventEmitter());
    __publicField(this, "_wsClose", (ev) => this._ev.emit("close", ev));
    __publicField(this, "_wsError", (ev) => this._ev.emit("error", ev));
    __publicField(this, "_wsMessage", (ev) => this._ev.emit("message", ev));
    __publicField(this, "_wsOpen", (ev) => this._ev.emit("open", ev));
    __publicField(this, "wait", () => {
      this._throwIfClosed();
      return new Promise((resolve, reject) => {
        ...
        this._ev.once("close", ({ code, reason }) => {
          ...
          const err = new TunnelError(code, reason.toString());
          reject(err);
        });
      });
    });
    __publicField(this, "close", (code, reason) => {
      this._throwIfClosed();
      this._closed = true;
      this._ws.close(code != null ? code : CLOSE_CODES.NORMAL_CLOSURE, reason);
    });
    __publicField(this, "hello", () => {
      this._throwIfClosed();
      const hello = { type: "hello" };
      this._ws.send(JSON.stringify(hello));
    });
    __publicField(this, "_throwIfClosed", () => {
      if (this._closed) {
...
var TunnelTail = class _TunnelTail extends TunnelClient {
  constructor(host, tunnelId) {
    const url = formatUrl(host, tunnelId);
    const headers = { "User-Agent": "tunnel-client" };
    const socket = import_browser_or_node.isBrowser ? new browser_default(url) : new browser_default(url, { headers });
    super(socket);
    __publicField(this, "events", this._ev);
    __publicField(this, "send", (response) => {
      this._throwIfClosed();
      const res = __spreadValues({ type: "response" }, response);
      this._ws.send(JSON.stringify(res));
    });
    __publicField(this, "_parseMessage", (ev) => {
    ...
    this._ev.on("message", (ev) => {
      const message = this._parseMessage(ev);
      if (!message) {
        this.close(CLOSE_CODES.INVALID_REQUEST_PAYLOAD, "invalid request payload");
        return;
      }
  ...
  static new(host, tunnelId) {
    const inst = new _TunnelTail(host, tunnelId);
    return new Promise((resolve, reject) => {
...
var TunnelHead = class extends TunnelClient {
  constructor(tunnelId, ws2) {
    super(ws2);
    this.tunnelId = tunnelId;
    __publicField(this, "events", this._ev);
    __publicField(this, "send", (request) => {
      this._throwIfClosed();
      const req = __spreadValues({ type: "request" }, request);
    ...
    this._ev.on("message", (ev) => {
      const message = this._parseMessage(ev);
      if (!message) {
        this.close(CLOSE_CODES.INVALID_RESPONSE_PAYLOAD, "invalid response payload");
        return;
      }
      if (message.type === "hello") {
        this.events.emit("hello", {});
...
var TunnelServer = class _TunnelServer {
  constructor(props) {
    __publicField(this, "_wss");
    __publicField(this, "_tunnels", {});
    __publicField(this, "_closed", false);
    ...
    __publicField(this, "getTunnel", (tunnelId) => {
      this._throwIfClosed();
    ...
    __publicField(this, "_handleConnection", (ws2, req) => {
      const parseResult = parseUrl(req.url);
      if (parseResult.status === "error") {
        const code = CLOSE_CODES.INVALID_TUNNEL_ID;
        const { reason } = parseResult;
        ws2.close(code, reason);
        return;
      }
      const { tunnelId } = parseResult;
      const tunnel = new TunnelHead(tunnelId, ws2);
    ...
    __publicField(this, "_handleDisconnection", (tunnelId) => {
    ...
    if (!browser_default.WebSocketServer) {
      throw new Error("Cannot instantiate TunnelServer in the browser");
    }
    this._wss = new browser_default.WebSocketServer({ server: props.server, port: props.port });
  ...
  static new(props) {
    const inst = new _TunnelServer(props);
    if (props.server) {
